...
for (var g = 0; g < 20; g++) {
    for (var h = 0; h < 20; h++) {
        stars[g] = new star(Rune.random(70, 130) * g, Rune.random(70,130) * h, 20);
    }
}
...
//backpack
var backpackwidth = faceWidth * 1.4;
...
function star(moveX, moveY, length) {
    ...
    r.rect(0, 0, 1, length, stargroup).fill(white).stroke(false).strokeCap("round");
    r.rect(0, 0, 1, length, stargroup).fill(white).stroke(false).rotate(270).strokeCap("round");
    r.rect(0, 0, 1, length, stargroup).fill(white).stroke(false).rotate(90).strokeCap("round");
    r.rect(0, 0, 1, length, stargroup).fill(white).stroke(false).rotate(180).strokeCap("round");
...
var eyes = r.group(0,0)
    r.rect(r.width / 2 - (faceWidth*.9 / 2), r.height / 2 - (faceHeight / 2), faceWidth*.9, faceHeight).strokeCap("round").strokeJoin("round").fill(false).stroke(white).strokeWidth(4);
