...
for (var i = 0; i < raindropCols; i++) {
    for (var j = 0; j < RndmRowNum; j++) {
        ...
        var moveDown = Rune.random(40, 60);
        if (i % 2 == 0) {
            oddAdder = moveLeft / 3;
        } else {
            oddAdder = 0;
        }
        raindrops[i] = new Raindrop(startX + oddAdder + 500 + moveLeft * j, startY - moveDown * i, startX + oddAdder + 500 + moveLeft * j - lineWidth, startY - moveDown * i + lineLength, Rune.random(.2, 6), Rune.random(20, 100), Rune.random(20, 100), Rune.random(1, 5));
...
for (var i = 0; i < 6; i++) {
    ...
    if (i > 3) {
        var lineheight = startY + 700;

    } else {
        var lineheight = startY + 300;
    }
...
function Raindrop(lineStartx, lineSarty, lineEndx, LineEndy, lineStroke, dash1, dash2, randomRingAmt) {
    ...
    for (var i = 0; i < randomRingAmt; i++) {
        ...
        for (var j = 0; j < points; j++) {
            ...
            var y = .3 * Math.sin(Rune.radians(j * spacing)) * i * randomRadius;
