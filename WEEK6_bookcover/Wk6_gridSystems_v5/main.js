...
f.load(function(err) {
    ...
    //main background to fill levers with
    ...
    for (var i = 0; i < res.length; i++) {
        ...
        var path = f.toPath(res[i], fontsize * (i % lettersinRow) + noise.get(noiseStep), fontsize * m, fontsize, true).fill(false).stroke(100).strokeWidth(5).strokeJoin("round")
        if (res[i] == ' ') {
        ...
        for (var i = 0; i < 200; i++) {
            ...
            if (pos) {
                r.circle(pos.x,pos.y,3,3).fill(255,0,0)
                console.log(pos.x);
            }
        }
        Title.push(path);
